defmodule Imagerepository.AccountTest do
  ...
  describe "images" do
    ...
    @valid_attrs %{caption: "some caption", tag: "some tag"}
    @update_attrs %{caption: "some updated caption", tag: "some updated tag"}
    ...
    test "list_images/0 returns all images" do
      image = image_fixture()
      assert Account.list_images() == [image]
    end

    test "get_image!/1 returns the image with given id" do
      image = image_fixture()
      assert Account.get_image!(image.id) == image
    end
    ...
    test "update_image/2 with invalid data returns error changeset" do
      image = image_fixture()
      assert {:error, %Ecto.Changeset{}} = Account.update_image(image, @invalid_attrs)
      assert image == Account.get_image!(image.id)
    end
